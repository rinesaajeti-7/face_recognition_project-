...
def img2tensor(img, bgr2rgb=False):
    """
    将图像转换为 PyTorch 张量。

    Parameters:
        img (PIL.Image or np.ndarray): 输入的图像，可以是 PIL 图片或 NumPy 数组。
        bgr2rgb (bool): 如果为 True，表示将图像从 BGR 转换为 RGB，默认是 False。

    Returns:
        torch.Tensor: 转换后的张量，形状为 (C, H, W)。
    """
    # 如果输入是 PIL 图片，将其转换为 NumPy 数组
    if isinstance(img, Image.Image):
        img = np.array(img)
    
    # 如果输入是 NumPy 数组，确保是 uint8 类型
    if img.dtype != np.uint8:
        img = img.astype(np.uint8)
    
    # 如果需要 BGR 转 RGB
    if bgr2rgb:
        img = img[..., ::-1].copy()  # 反转最后一个维度，从 BGR 转为 RGB

    # 转换为张量
    tensor = torch.from_numpy(img).float()  # 转换为 float 类型
    tensor = tensor.permute(2, 0, 1)  # 转换维度为 (C, H, W)

    return tensor

def tensor2img(tensor, rgb2bgr=False):
    """
    将 PyTorch 张量转换为图像。

    Parameters:
        tensor (torch.Tensor): 输入的张量，形状为 (C, H, W) 或 (B, C, H, W)。
        rgb2bgr (bool): 如果为 True，表示将图像从 RGB 转换为 BGR，默认是 False。
    ...
    img = tensor.permute(1, 2, 0).byte().numpy()

    # 如果需要从 RGB 转换为 BGR
    if rgb2bgr:
        img = img[..., ::-1].copy()  # 反转最后一个维度
...
def sample_dpmpp_2m(model, x, sigmas, extra_args=None, callback=None, disable=None):
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        denoised = model.denoise(x, sigmas[i] * s_in, **extra_args)
