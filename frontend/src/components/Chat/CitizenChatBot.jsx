...
const CitizenChatBot = ({ citizenId = null }) => {
  ...
  useEffect(() => {
    if (isOpen && !hasWelcome) {
      const welcomeMessage = "👋 Përshëndetje! Unë jam asistenti për qytetarët. Si mund t'ju ndihmoj?";
  ...
  const sendMessage = useCallback(async (messageText = input) => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/chat/citizen', {
        message: messageText,
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="chat-window citizen-chat">
          <div className="chat-header">
            <div className="chat-header-info">
              <div className="chat-avatar">🤖</div>
              <div>
                <h3>Asistenti për Qytetarët</h3>
